...
class LeRobotDatasetMetadata:
    ...
    def load_metadata(self):
        ...
        logging.debug(f"Loaded tasks in {time.time() - start:.3f}s")
        
        start = time.time()
        self.episodes = load_episodes(self.root)
        episodes_time = time.time() - start
        logging.info(f"Loaded episodes in {episodes_time:.3f}s")
        
        start = time.time()
        self.stats = load_stats(self.root)
        logging.debug(f"Loaded stats in {time.time() - start:.3f}s")
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _check_cached_episodes_sufficient(self) -> bool:
        ...
        # Early return if no episodes specified - no need to check
        if self.episodes is None:
            return True
